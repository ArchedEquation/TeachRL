...
### The Proof: Training vs Just Prompting

A natural question after seeing the self-play results is: *how much of this is the training, and how much is just the underlying model capability?*

To answer that directly, we ran Qwen-72B — one of the strongest open-source LLMs available — on the same self-play escalation task with **zero training**. Just a system prompt explaining the archetypes, the teaching rules, and the observation format. No gradient updates. No environment interaction during learning. Pure zero-shot prompting.

Here are the episode-by-episode scores:

| Episode | PPO (trained on TeachRL) | Qwen-72B (zero-shot) |
|---|---|---|
| 1 | 0.981 | 0.252 |
| 2 | 0.819 | 0.414 |
| 3 | 0.713 | 0.397 |
...
**+0.469 gap. That difference is entirely the environment and training pipeline.**

The LLM is not a bad agent. It reads the observation, reasons about what the student needs, and picks a teaching action. But it has no memory of what strategies worked before. It cannot adapt when the escalator tightens the parameters between episodes. It collapses early — episodes 1, 3, and 7 end before the step budget because engagement hits zero, something the PPO agent almost never allows.

The PPO agent, by contrast, learned three things from environment interaction that the LLM never did:

- **Engagement management** — never let it hit zero, back off difficulty before it collapses
- **Concept switching tempo** — how often to switch vs drill, calibrated per archetype
- **Escalation adaptation** — when the environment gets harder, the PPO agent adjusts; the LLM restarts from first principles every episode

This is the core argument for why the environment matters: a large, capable LLM prompted carefully is still outperformed by +39% on this task by a much smaller PPO policy that simply had more interactions with the right training signal.
