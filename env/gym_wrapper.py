"""env/gym_wrapper.py — Gymnasium wrapper for SB3/PPO training."""
...
N_DIFFS    = len(DIFFICULTY_LEVELS)
N_ACTIONS  = N_CONCEPTS * N_DIFFS   # 30
OBS_DIM    = N_CONCEPTS * 4 + 5     # 45: per-concept(sr,att,streak,prereq) + eng,fat,steps,hint_rel,last_correct

CONCEPT_IDX = {c: i for i, c in enumerate(CONCEPTS)}
DIFF_IDX    = {d: i for i, d in enumerate(DIFFICULTY_LEVELS)}

def action_to_int(c: str, d: str) -> int:
    return CONCEPT_IDX[c] * N_DIFFS + DIFF_IDX[d]
...
class TeachRLGymEnv(gym.Env):
    ...
    def __init__(self, task_id: str = "blind_teaching",
                 seed: int = 42, render_mode: Optional[str] = None):
        super().__init__()
    ...
    def step(self, action: int):
        ...
        result = self._env.step({"concept": c, "difficulty": d,
